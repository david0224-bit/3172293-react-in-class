...
import { Infiniity as InfinityLoader } from "ldrs/react";
import "ldrs/react/Infinity.css";
import { file, url } from "zod";

export default function FileInput({
    value = [], // estado ecterno (files)
    onChange, // setter externo
    multiple = false, // modo seleccion
    accept = "image/*,appplication/pdf", // tipos permitidos
})  {
    const inputRef = useRef();
    const [isLoading, setIsLoading] = useState(false); // loader
    const [dragIndex, setDragIndex] = useState(null); // indice drag

    const isImage = (file) => file.type.startsWith("image/"); // discriminador MINE

    // Genera preview SOLO para imagenes (evita crea URLs innecesarias)
    const previews = useMemo(
        () =>
            value.map((file) => (isImage(file) ? URL.createObjectURL(file) : null)), [value],
    );
        ...
        <div className="flex items-center gap-2">
            {value.map((file, i) => (
                <div
                    ...
                    className="realtive w-24 h-24 border rounded overflow-hidden group"
                >
